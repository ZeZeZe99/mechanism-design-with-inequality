...
class Myerson:
    ...
    def __init__(self, pop, q, rev_max=True):
        self.build_primal(pop, q, rev_max)
        self.build_dual(pop, q, rev_max)

    """
    Build the primal LP model
    """
    def build_primal(self, pop, q, rev_max=True):
        ...
        if rev_max:
            self.primal.setObjective(
                gp.quicksum(p[i] * pop.pdf[i] for i in range(pop.num_type)),
                GRB.MAXIMIZE)
        # maximizing surplus
        else:
            self.primal.setObjective(
                gp.quicksum(pop.vsList[i] * x[i] * pop.pdf[i] for i in range(pop.num_type)),
                GRB.MAXIMIZE)
        ...
        self.primal.addConstrs((pop.vsList[i] * x[i] - p[i] >= 0 for i in range(pop.num_type)),
                               "IR")
        # supply
        self.primal.addConstr(gp.quicksum(x[i] * pop.pdf[i] for i in range(pop.num_type)) <= q, "supply")
        # bound
    ...
    def build_dual(self, pop, q, rev_max):
